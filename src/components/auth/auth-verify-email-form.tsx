...
export function AuthVerifyEmailForm() {
  ...
  return (
    <div className="bg-background relative min-h-screen overflow-hidden">
      <div className="pointer-events-none absolute -top-8 -left-10 md:top-auto md:bottom-0 md:left-0">
        <Image
          src="/images/wave.png"
          alt=""
          width={240}
      ...
      <div className="absolute inset-x-0 top-6 z-10 flex justify-center gap-2 md:inset-x-auto md:top-8 md:left-8 md:justify-start">
      ...
      <div className="flex min-h-screen items-center justify-center px-4 py-24 md:items-start md:pt-28 md:pb-20">
        <div className="w-full max-w-131.5 text-center">
          <h1 className="text-foreground text-[2.25rem] leading-tight font-semibold tracking-[-0.02em]">
            Email Verification
          </h1>

          <p className="text-grey-dark mt-4 text-base leading-8">
            Hello Amaka, enter the 6-digit code sent to your email to verify and activate
            your EnergyIQ account.
          </p>
          ...
          <div className="mt-10 px-4">
                ...
                <InputOTPGroup className="gap-2">
                  {Array.from({ length: 6 }).map((_, index) => (
                    <InputOTPSlot
                      ...
                      index={index}
                      className={cn(
                        "h-10 w-10 rounded-lg border bg-transparent text-lg font-medium shadow-none transition-colors",
                        "data-[active=true]:border-positive data-[active=true]:ring-positive/20 data-[active=true]:ring-[3px]",
                        "focus:ring-0",
                        isError
                          ? "border-danger text-danger data-[active=true]:border-danger data-[active=true]:ring-danger/20"
                          : isSuccess
                            ? "border-positive text-positive"
                            : "border-slate-70 focus:border-primary"
                      )}
                    />
                  ))}
                </InputOTPGroup>
            ...
            <div className="min-h-16">
              {isError ? (
                <div className="bg-danger-bg text-danger mt-8 flex items-center justify-center gap-2 rounded-lg py-3 text-lg font-medium">
                  <span>{errorMessage}</span>
                </div>
              ) : isSuccess ? (
                <div className="bg-success-bg text-positive mt-8 flex items-center justify-center gap-2 rounded-lg py-3 text-lg font-medium">
          ...
          <div className="mt-14 flex w-full flex-col gap-4 md:grid md:grid-cols-2 md:gap-6">
            ...
            >
              {verifyMutation.isPending ? (
                ...
                <>
                  <span className="md:hidden">Verify my email</span>
                  <span className="hidden md:inline">Confirm Email</span>
                </>
              )}
            </Button>
            ...
            <Button
              ...
              size="lg"
              asChild
              className="h-14 rounded-xl border border-slate-200 bg-transparent text-lg font-medium text-slate-900 shadow-none transition-colors hover:bg-slate-50 md:order-1"
            >
              <Link href="/signup">
                <span className="md:hidden">Back to sign up</span>
      ...
      {isSuccess && (
        <div className="animate-in fade-in fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-[2px] duration-300">
          ...
          >
            ...
            <p className="text-grey-dark mb-8 text-base md:text-lg md:whitespace-nowrap">
