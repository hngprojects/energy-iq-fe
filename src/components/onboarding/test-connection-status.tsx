...
export function TestConnectionStatus({
    ...
    return (
      <div className="rounded-xl bg-[#FFE8E8] px-4 py-3">
        ...
        <div className="mt-2 flex items-center gap-2 text-xs text-red-600">
          ...
          <span>Could not authenticate. Check your credentials and try again.</span>
        </div>
      </div>
    )
  }
    ...
    return (
      <div>
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <span className={dotClass} />
            <span className="text-base font-medium text-[#111827]">{label}</span>
          </div>
          <HugeiconsIcon icon={ArrowRight02Icon} className="size-5 text-[#2A2F3C]" />
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <span className={dotClass} />
        <span className="text-base font-semibold text-[#2A2F3C] lg:text-lg">{label}</span>
      </div>
      <HugeiconsIcon icon={ArrowRight02Icon} className="size-5 text-[#2A2F3C]" />
    </button>
  )
